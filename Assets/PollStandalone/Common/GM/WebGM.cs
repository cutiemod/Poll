...
public class WebGM : MonoBehaviour
{
    ...
    public WebSC sceneChangerComponent;
    ...
    internal void GoToScene(string sceneName = "", float speedMultiplier = 1, bool killAudio = false)
    {
        sceneChangerComponent.transform.position = transform.position;
        sceneChangerComponent.FadeToScene(sceneName, speedMultiplier, killAudio);
    }

    //internal void GetNewSceneChangerObject()
    //{
    //    sceneChangerComponent = GameObject.Find(sceneChangerObjectName).GetComponent<SceneChanger>();
    //}

    internal void FindNewCamera()
    {
        winScreen.SetActive(false);

        transform.position = Vector3.zero;
        sceneChangerComponent.transform.position = Vector3.zero;

        guiCanvas.worldCamera = Camera.main; //FindObjectOfType<Camera>();
        guiCanvas.sortingLayerName = "Overlay";
    }
    #endregion
